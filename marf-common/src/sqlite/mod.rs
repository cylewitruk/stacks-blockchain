...
use rusqlite::{OpenFlags, Connection, Transaction, Error as SqliteError};

use crate::{
    storage::{TrieIndexProvider, TrieFileStorageTrait, TrieFile, TrieStorageTransientData, TrieFileStorage, TrieStorageConnection, TrieStorageTransaction},
    MarfTrieId, MarfError, MarfOpenOpts, TrieCache, diagnostics::TrieBenchmark, utils::Utils
};
...
impl<TTrieId: MarfTrieId> SqliteTrieFileStorage<TTrieId> {
    ...
    fn open_opts(
        ...
        if create_flag {
            trie_sql::create_tables_if_needed(&mut &self.db)?;
        }
        ...
        let prev_schema_version = trie_sql::migrate_tables_if_needed::<TTrieId>(&mut self.db)?;
        ...
        if trie_sql::detect_partial_migration(&index)? {
...
impl<TTrieId: MarfTrieId, SqliteIndexProvider> TrieFileStorageTrait<TTrieId, SqliteIndexProvider> for SqliteTrieFileStorage<TTrieId> {
    fn connection<'a>(&'a mut self) -> crate::storage::TrieStorageConnection<'a, TTrieId, SqliteIndexProvider> {
    ...
    fn transaction<'a>(&'a mut self) -> Result<crate::storage::TrieStorageTransaction<'a, TTrieId, SqliteIndexProvider>, crate::MarfError> {
        if self.readonly() {
    ...
    fn open(db_path: &str, marf_opts: crate::MarfOpenOpts) -> Result<crate::storage::TrieFileStorage<TTrieId, SqliteIndexProvider>, crate::MarfError> {
        SqliteTrieFileStorage::open_opts(db_path, false, false, marf_opts)
    }

    fn open_readonly(
        db_path: &str,
        marf_opts: crate::MarfOpenOpts,
    ) -> Result<crate::storage::TrieFileStorage<TTrieId, SqliteIndexProvider>, crate::MarfError> {
        SqliteTrieFileStorage::open_opts(db_path, true, false, marf_opts)
    }
    ...
    fn open_unconfirmed(
        ...
        SqliteTrieFileStorage::open_opts(db_path, false, true, marf_opts)
    }

    fn reopen_readonly(&self) -> Result<crate::storage::TrieFileStorage<TTrieId, SqliteIndexProvider>, crate::MarfError> {
