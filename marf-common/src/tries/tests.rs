...
use stacks_common::{util::hash::to_hex, types::chainstate::{TrieHash, BlockHeaderHash}};

use crate::{
    marf::*,
    storage::*,
    ClarityMarfTrieId,
    *, tries::{TrieHashCalculationMode, TriePath, TrieLeaf, TrieNodeType, TrieCursor, TrieMerkleProof}, utils::Utils
};

use super::{TrieNodeID, TriePtr, TrieNode256, TrieNode48, TrieNode16, TrieNode4, TrieNode};
    ...
    for marf_opts in MarfOpenOpts::all().into_iter() {
        ...
        .iter()
        {
            ...
            for i in 0..32 {
                ...
                match node {
                    TrieNodeType::Node4(ref data) => assert_eq!(Utils::count_children(&data.ptrs), 2),
                    TrieNodeType::Node16(ref data) => assert_eq!(Utils::count_children(&data.ptrs), 2),
                    TrieNodeType::Node48(ref data) => assert_eq!(Utils::count_children(&data.ptrs), 2),
                    TrieNodeType::Node256(ref data) => {
                        assert_eq!(Utils::count_children(&data.ptrs), 2)
                    }
                    _ => assert!(false),
                };
            }
...
fn trie_cursor_promote_leaf_to_node4() {
    for marf_opts in MarfOpenOpts::all().into_iter() {
        ...
        for i in 0..31 {
            ...
            match node {
                TrieNodeType::Node4(ref data) => assert_eq!(Utils::count_children(&data.ptrs), 2),
                TrieNodeType::Node256(ref data) => assert_eq!(Utils::count_children(&data.ptrs), 2),
                _ => assert!(false),
            };
        }
        ...
        for ptr in ptrs.iter() {
            ...
            match node {
                TrieNodeType::Node16(ref data) => {
                    assert_eq!(Utils::count_children(&data.ptrs), 5);
                }
                _ => {
                    assert!(false);
                }
        ...
        for ptr in ptrs.iter() {
            ...
            match node {
                TrieNodeType::Node48(ref data) => {
                    assert_eq!(Utils::count_children(&data.ptrs), 17);
                }
                _ => {
                    assert!(false);
                }
...
fn trie_cursor_promote_node48_to_node256() {
    for marf_opts in MarfOpenOpts::all().into_iter() {
        ...
        for ptr in ptrs.iter() {
            ...
            match node {
                TrieNodeType::Node256(ref data) => {
                    assert_eq!(Utils::count_children(&data.ptrs), 49);
                }
                _ => {
                    assert!(false);
                }
