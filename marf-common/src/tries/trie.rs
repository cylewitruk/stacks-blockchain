...
impl Trie {
    ...
    fn read_root_maybe_hash<T: MarfTrieId>(
        ...
        let ptr = TriePtr::new(
            Utils::set_backptr(TrieNodeID::Node256 as u8),
            0,
            storage.root_ptr(),
        );
    ...
    pub fn walk_from_nohash<T: MarfTrieId>(
        ...
        node: &TrieNodeType,
        cursor: &mut TrieCursor<T>,
    ) -> Result<Option<(TriePtr, TrieNodeType)>, MarfError> {
    ...
    fn replace_leaf<T: MarfTrieId>(
        ...
        let leaf_hash = Utils::get_leaf_hash(value);
    ...
    fn promote_leaf_to_node4<T: MarfTrieId>(
        ...
        let cur_leaf_hash = Utils::get_leaf_hash(cur_leaf_data);
        ...
        let new_leaf_hash = Utils::get_leaf_hash(new_leaf_data);
        ...
        let node4_hash = Utils::get_node_hash(
            &node4_data,
            &vec![
                cur_leaf_hash,
                new_leaf_hash,
    ...
    fn try_attach_leaf<T: MarfTrieId>(
        ...
        let new_node_hash = Utils::get_nodetype_hash(storage, node)?;
    ...
    fn insert_leaf<T: MarfTrieId>(
        ...
        let new_node_hash = Utils::get_nodetype_hash(storage, &new_node)?;
    ...
    fn splice_leaf<T: MarfTrieId>(
        ...
        let leaf_hash = Utils::get_leaf_hash(leaf);
    ...
    fn recalculate_root_hash<T: MarfTrieId>(
        ...
        if ptrs.len() == 0 {
            ...
            let my_hash = Utils::get_nodetype_hash(storage, &node)?;
            ...
            while let Some(ptr) = ptrs.pop() {
                ...
                let content_hash = Utils::get_nodetype_hash(storage, &node)?;
                ...
                child_ptr = ptr;
                child_ptr.id = Utils::clear_backptr(child_ptr.id);
            }
        }
        // must be at the root
